...
function concatJsTask() {
  return src('./js/*.js').pipe(concat('scripts.js')).pipe(dest('./js'));
}

//複数のJavascriptファイルをまとめる
function browserifyTask() {
  return browserify('./js/scripts.js').bundle().pipe(source('scripts.js')).pipe(buffer()).pipe(dest('./js'));
}
...
function browsersyncServe(cb) {
  browsersync.init({
    proxy: 'http://chaga.wp/', // Local by Flywheelのドメインを入力。最後の/も入れる
    open: true,
    watchOptions: {
...
//本番環境では不要なタスク
exports.prod = series(scssTask, concatJsTask, browserifyTask, jsProdTask, cleanTask, imageminTask);
